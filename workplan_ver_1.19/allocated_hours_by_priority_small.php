 ...
 */
    ...
    class allocated_hours_by_priority_smallPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'allocated hours by priority small');
            $field = new StringField('priority');
            $this->dataset->AddField($field, false);
            $field = new StringField('total_allocated_hrs');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('allocated_hours_by_priority_smallssearch', $this->dataset,
                array('priority', 'total_allocated_hrs'),
                array($this->RenderText('Priority'), $this->RenderText('Total Allocated Hrs')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('allocated_hours_by_priority_smallasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('total_allocated_hrs', $this->RenderText('Total Allocated Hrs')));
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for total_allocated_hrs field
            //
            $editor = new TextEdit('total_allocated_hrs_edit');
            $editColumn = new CustomEditColumn('Total Allocated Hrs', 'total_allocated_hrs', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function allocated_hours_by_priority_smallGrid_OnGetCustomTemplate($part, $mode, &$result, &$params)
        {
        if ($part == PagePart::Grid && $mode == PageMode::ViewAll)
          $result = 'hours_by_priority_small.tpl';
        }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'allocated_hours_by_priority_smallGrid');
            ...
            $result->SetWidth('');
            $this->OnGetCustomTemplate->AddListener('allocated_hours_by_priority_smallGrid' . '_OnGetCustomTemplate', $this);
    ...
    try
    {
        $Page = new allocated_hours_by_priority_smallPage("allocated_hours_by_priority_small.php", "allocated_hours_by_priority_small", GetCurrentUserGrantForDataSource("allocated hours by priority small"), 'UTF-8');
        $Page->SetShortCaption('Est Hours By Priority dashboard');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Allocated Hours By Priority Small');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("allocated hours by priority small"));
