 ...
 */
    ...
    class Hours_per_Project_by_userPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'Hours per Project by user');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('Hours_per_Project_by_userssearch', $this->dataset,
                array('name', 'total_hours', 'project_name', 'task_name'),
                array($this->RenderText('Name'), $this->RenderText('Actual Hours'), $this->RenderText('Project Name'), $this->RenderText('Task Name')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('Hours_per_Project_by_userasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('name', $this->RenderText('Name')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('total_hours', $this->RenderText('Actual Hours')));
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new CustomEditColumn('Actual Hours', 'total_hours', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
        ...
        public function Hours_per_Project_by_userGrid_OnGetCustomTemplate($part, $mode, &$result, &$params)
        {
        if ($part == PagePart::Grid && $mode == PageMode::ViewAll)
          $result = 'hours_per_project_by_user.tpl';
        }
        public function Hours_per_Project_by_userGrid_OnCustomDrawRow($rowData, &$rowCellStyles, &$rowStyles)
        {
        if ($rowData['total_hours']  > $rowData['assigned_hours'])
        $rowCellStyles['total_hours'] .= 'font-size: 10pt;font-weight: bold; color: #FFFFFF;background-color:#FF0000;';
        }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'Hours_per_Project_by_userGrid');
            ...
            $result->SetWidth('');
            $this->OnGetCustomTemplate->AddListener('Hours_per_Project_by_userGrid' . '_OnGetCustomTemplate', $this);
            $result->OnCustomDrawCell->AddListener('Hours_per_Project_by_userGrid' . '_OnCustomDrawRow', $this);
    ...
    try
    {
        $Page = new Hours_per_Project_by_userPage("Hours per project by user.php", "Hours_per_Project_by_user", GetCurrentUserGrantForDataSource("Hours per Project by user"), 'UTF-8');
        $Page->SetShortCaption('Hours per Project');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Hours per project');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("Hours per Project by user"));
