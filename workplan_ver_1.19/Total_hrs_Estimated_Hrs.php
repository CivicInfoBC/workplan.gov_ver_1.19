 ...
 */
    ...
    class Total_hrs_Estimated_HrsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'Total hrs Estimated Hrs');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('Total_hrs_Estimated_Hrsssearch', $this->dataset,
                array('name', 'actual_hours'),
                array($this->RenderText('Name'), $this->RenderText('Actual Hours')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('Total_hrs_Estimated_Hrsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        public function Total_hrs_Estimated_HrsGrid_OnGetCustomTemplate($part, $mode, &$result, &$params)
        {
        if ($part == PagePart::Grid && $mode == PageMode::ViewAll)
          $result = 'actual_hours_versus_allocated.tpl';
        }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'Total_hrs_Estimated_HrsGrid');
            ...
            $result->SetWidth('');
            $this->OnGetCustomTemplate->AddListener('Total_hrs_Estimated_HrsGrid' . '_OnGetCustomTemplate', $this);
    ...
    try
    {
        $Page = new Total_hrs_Estimated_HrsPage("Total_hrs_Estimated_Hrs.php", "Total_hrs_Estimated_Hrs", GetCurrentUserGrantForDataSource("Total hrs Estimated Hrs"), 'UTF-8');
        $Page->SetShortCaption('Chart: Actual Hrs / Assigned Hrs');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Actual Hrs versus Assigned Hrs');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("Total hrs Estimated Hrs"));
