 ...
 */
    ...
    class task_namesPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('task_namesssearch', $this->dataset,
                array('Name'),
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('task_namesasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('task_namesGrid_Name_handler_list');
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('task_namesGrid_Name_handler_view');
        ...
        public function GetModalGridDeleteHandler() { return 'task_names_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'task_namesGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('Name', 'Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'task_namesGrid_Name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for Name field
            //
            $column = new TextViewColumn('Name', 'Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'task_namesGrid_Name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new task_namesPage("task_names.php", "task_names", GetCurrentUserGrantForDataSource("task_names"), 'UTF-8');
        $Page->SetShortCaption('Task Names');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Task Names');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("task_names"));
