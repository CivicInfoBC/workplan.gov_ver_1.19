...
$dsn = 'mysql:dbname=wp12;host=localhost';
$user = 'root';
$password = 'root';
...
//start total allocated hours per user
...
//Query for "total Assigned hours per project"
$QueryResult6 = $pdo->query('SELECT SUM(ts.total_hours) AS `total_hours`,
       p.`project_name`,
       SUM(t.`hrs`) AS `assigned_hours`,`task_name`
FROM `task` t INNER JOIN
      `project` p
       ...
       ON t.`task_id` = ts.`task_id`
GROUP BY p.`project_name`');
foreach ($QueryResult6 as $q6row) {

$q6rows[] = "['{$q6row['project_name']}',{$q6row['assigned_hours']}]";

 
}
$q6rowsString = implode(',',$q6rows);
...
var options6 = {title: '',is3D:true, height:300 };
...
<!-- Charts Widget -->
			<div id="wgt-charts1" class="jdash-widget">
				<div class="jdash-header">Total Allocated Hours<span class="subtitle">Allocated Hours per Project</span></div>
				<div class="jdash-body"><div id="chart_div6" ></div></div>
			</div>
