 ...
 */
    ...
    class actual_hours_smallPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'actual hours small');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('actual_hours_smallssearch', $this->dataset,
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('actual_hours_smallasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        public function actual_hours_smallGrid_OnGetCustomTemplate($part, $mode, &$result, &$params)
        {
        if ($part == PagePart::Grid && $mode == PageMode::ViewAll)
          $result = 'staff_hours_small.tpl';
        }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'actual_hours_smallGrid');
            ...
            $result->SetWidth('');
            $this->OnGetCustomTemplate->AddListener('actual_hours_smallGrid' . '_OnGetCustomTemplate', $this);
    ...
    try
    {
        $Page = new actual_hours_smallPage("actual_hours_small.php", "actual_hours_small", GetCurrentUserGrantForDataSource("actual hours small"), 'UTF-8');
        $Page->SetShortCaption('Actual Hours dashboard');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Actual Hours Small');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("actual hours small"));
