...
var n=d.getUTCMonth();
  ...
  dataTable.addColumn({ type: 'date', id: 'Project_Start' });
  dataTable.addColumn({ type: 'date', id: 'Action_Start' });
  dataTable.addColumn({ type: 'date', id: 'Project_End' });
  dataTable.addColumn({ type: 'date', id: 'Action_End' });
  ...
  {/literal}
   	{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
			...
			['{$Row.DataCells.task_name.Data}', new Date({$Row.DataCells.task_date_start.Data}), new Date({$Row.DataCells.task_date_end.Data})],
        {/foreach}
		 {literal}
	]);
...
<div id="time_line" style="width: 900px; height: 480px;"></div>
