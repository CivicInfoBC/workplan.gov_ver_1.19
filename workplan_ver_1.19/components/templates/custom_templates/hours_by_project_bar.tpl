...
<script type="text/javascript">
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ['Project Name', 'Actual Hours', 'Assigned Hours']
        {/literal}
        {foreach item=Row from=$DataGrid.Rows name=RowsGrid}
            ,['{$Row.DataCells.project_name.Data}', {$Row.DataCells.total_hours.Data},{$Row.DataCells.assigned_hours.Data}]
        {/foreach}
        {literal}
        ]);

        var options = {
            title: 'Actual hours per Project and Assigned hours per project'
        };
