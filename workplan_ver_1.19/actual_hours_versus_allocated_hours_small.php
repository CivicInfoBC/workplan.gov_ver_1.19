 ...
 */
    ...
    class actuall_hours_versus_allocated_hours_smallPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'actuall hours versus allocated hours small');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('actuall_hours_versus_allocated_hours_smallssearch', $this->dataset,
                array('name', 'actual_hours', 'assigned_hrs'),
                array($this->RenderText('Name'), $this->RenderText('Actual Hours'), $this->RenderText('Assigned Hrs')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('actuall_hours_versus_allocated_hours_smallasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('name', $this->RenderText('Name')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('actual_hours', $this->RenderText('Actual Hours')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('assigned_hrs', $this->RenderText('Assigned Hrs')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for actual_hours field
            //
            $column = new TextViewColumn('actual_hours', 'Actual Hours', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for assigned_hrs field
            //
            $column = new TextViewColumn('assigned_hrs', 'Assigned Hrs', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for actual_hours field
            //
            $editor = new TextEdit('actual_hours_edit');
            $editColumn = new CustomEditColumn('Actual Hours', 'actual_hours', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            ...
            //
            // Edit column for assigned_hrs field
            //
            $editor = new TextEdit('assigned_hrs_edit');
            $editColumn = new CustomEditColumn('Assigned Hrs', 'assigned_hrs', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function actuall_hours_versus_allocated_hours_smallGrid_OnGetCustomTemplate($part, $mode, &$result, &$params)
        {
        if ($part == PagePart::Grid && $mode == PageMode::ViewAll)
          $result = 'actual_hours_versus_allocated_small.tpl';
        }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'actuall_hours_versus_allocated_hours_smallGrid');
            ...
            $result->SetWidth('');
            $this->OnGetCustomTemplate->AddListener('actuall_hours_versus_allocated_hours_smallGrid' . '_OnGetCustomTemplate', $this);
    ...
    try
    {
        $Page = new actuall_hours_versus_allocated_hours_smallPage("actual_hours_versus_allocated_hours_small.php", "actuall_hours_versus_allocated_hours_small", GetCurrentUserGrantForDataSource("actuall hours versus allocated hours small"), 'UTF-8');
        $Page->SetShortCaption('Actual Hours vs Allocated Hours dashboard');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Actual Hours Versus Allocated Hours Small');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("actuall hours versus allocated hours small"));
